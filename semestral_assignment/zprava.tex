...
//TODO saturace na vzdálenost k překážce $\ge 0$.
...
Při uvážení $U_{bat}$ dle \ref{sec:ubat} to odpovídá vstupnímu napětí $U_{in} = \frac{60}{100} U_{bar} = 4.8$ \si{\volt}.
Změřená odezva je vykreslena na obrázku \ref{fig:motor_skok}.
...
\end{equation}
odpovídá úhlové rychlosti otáčení motoru při použité střídě $60$ \%. V čase $T$ musí mít asymptota nulovou hodnotu -- odečtením z grafu je vidět $T = 0.037$ \si{\second}.
Protože testovací vstupní signál neměl jednotkovou amplitudu, je potřeba nalezené parametry ještě přeškálovat vydělením $U_{in}$. 
...
\begin{equation}
    \begin{aligned}
        G_{u\to\theta}(s) &= \frac{1}{U_{in}} \frac{9.6}{s(0.037s + 1)}   &=  \frac{54.0541}{s(s+27.0270)}, \\
...
Rovnice \eqref{eq:pohyb1_easy} až \eqref{eq:motor2_easy} byly implementovány jako simulinkový model. Při simulaci byl nelineární i linearizovaný model
vystaven jednotkovému skoku vstupního napětí $u$ v čase 1 \si{\second}. Výstupy simulace jsou zobrazeny na obrázku \ref{fig:porovnani_skok},
...
V nelineárním modelu je zohledněna saturace náklonu $\psi$ na interval $[-\frac{\pi}{2}, \frac{\pi}{2}]$ rad. V čase $1.6 \si{\second}$ nastane pád robota na zem.
Nelineární systém dále setrvává ve stabilním ekvilibriu.
...
vykresleny odezvy na rampu se sklonem 1. 
\begin{figure}[htbp]
...
zatímco nelineární systém je v klidu až do času $300$ \si{\milli\second}, kdy napětí $u$ překoná hranici pásma necitlivosti $300$ \si{\milli\volt}.
...
\end{equation}
z nichž jedno je nestabilní. Za účelem plnění požadavků na chování bude potřeba před soustavu zařadit regulátor

\subsection{Balancování}
...
dvojice pólů $-0.0267 \pm 0.0521i$. Zbylé dva póly jsou ponechány na svém místě v bodech $-231$ a $0$.
...
\begin{equation*}
    ...
    \end{bmatrix},
\end{equation*}
jehož zapojení do obvodu je vidět na schématu \ref{fig:stavova_zv} zapůjčeném z \cite{img:full_state_feedback}.
