...
požadavky $T_s = 1.25~\text{\si{\second}}$ a $OS = 10~\%$ na póly $p_{1,2}$. Póly $p_{3,4}$ jsou ponechány v týchž polohách jako pro regulátor $K_1$.
Výsledný regulátor
\begin{equation*}
    K_3 = \begin{bmatrix}
        -0.1613 & -14.2147 &  -0.8729  & -2.0811
    \end{bmatrix}
\end{equation*}
zajistil jen zanedbatelné popojíždění segwaye při zajištění velice dobré stability a schopnosti se vypořádat s dynamickými změnami rozložení
...
takže jej lze plně využít pro návrh řízení. Detailní rozebrání několika možností regulace stavovou zpětnou vazbou umožňuje
...
přesně vyšetřit -- kritéria návrhu regulátorů zohledňující saturaci akčního zásahu jako nejpodstatnější statickou nelinearitu.
K vyzkoušení stojí regulace \textit{deadbeat}, jež byla pro využití v této práci zavržena kvůli celkově rychlé dynamice a velké nestabilitě
systému v otevřené smyčce. Ve druhé řadě by úloha stabilizace robota ve svislé poloze benefitovala z přesnějšího stanovení hranic pásem necitlivosti
...
zamlčený -- leč podstatný -- předpoklad identického pásma necitlivosti na všech dostupných motorech. Je nejvýše pravděpodobné,
že nasazení adaptivní metody kompenzace pásma necitlivosti by přispělo k hladkému postoji robota.

Protože je v případě modelování kinematiky dvoukolového vozítka označování stavů velmi intuitivní, je modelování systému stavovým popisem
doporučeníhodné i pro navazující rozvíjející publikace.  Stěžejním závěrem je samotné zdařilé splnění zadané úlohy.
