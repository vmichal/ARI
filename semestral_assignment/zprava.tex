...
Dále je v tabulce \ref{tab:konstanty} zanesena i hodnota maximálního napětí baterie $U_{max}$ po plném nabití
a minimální hodnota .
...
Na obrázku \ref{fig:pwm_saturace} je patrná první statická nelinearita: saturace akčního zásahu do soustavy,
jenž je omezen na interval $u \in [-U_{bat}, U_{bat}]$, kde $U_{bat}$ je okamžité napětí baterie.
Protože je použitá baterie dostatečně tvrdý zdroj a řízení motorů nevyžaduje příliš velký výkon, 
lze bez velké chyby postavit $U_{bat}$ = 8 \si{V}. Ve standardních podmínkách bude činnost segwaye ukončena dříve,
než baterie stihne poklesnout na napěťovou hladinu 8 \si{V}.
...
Hodnoty $\varphi = \pm \frac{\pi}{2}$ odpovídají stavu, kdy robot spadne a položí se na plocho na konstrukci těla.
Jedná se o stabilní ekvilibria, z hlediska řešení úlohy ale nemají význam. Pakliže je cílem udržet segway ve svislé poloze,
musí se soustava těmto dvěma stavům vyhnout za každou cenu.

Proměnná $x_1$ odpovídající úhlu koleček ani stavy $x_{3,4}$ odpovídající úhlovým rychlostem $\dot{\theta}$ a $\dot{\psi}$
obecně nemohou být saturovány. Úhlová dráha uražená kolečky je obecně neomezená, stejnětak obě úhlové rychlosti.
