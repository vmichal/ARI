...
\begin{figure}[htbp]
    \centerline{\includegraphics[width=\linewidth]{segway_shora.png}}
    \caption{Nákres segwaye shora, zapůjčeno z \cite{model_based_design}}
    \label{fig:segway_shora}        
\end{figure}

\begin{figure}[htbp]
    \centerline{\includegraphics[width=\linewidth]{segway_bok.png}}
...
nezbytných pro matematický popis segwaye. Celý matematický model je založen na \cite{model_based_design}.
Uvažujeme tři zobecnělé souřadnice
\begin{equation*}
    \begin{split}
        \frac{\theta_l + \theta_r}{2} = \theta &\cdots \text{Průměrný úhel levého a pravého kola} \\
        \psi &\cdots \text{Náklon segwaye (osa otáčení v ose koleček)} \\
        \varPhi &\cdots \text{Azimut segwaye (osa otáčení ve svislém směru).}
    \end{split}
\end{equation*}
...
\begin{table}[htbp]
    ...
    \begin{tabular}[t]{|c|c|c|}
        ...
        $ g $ & 9.81 m/s & Gravitační zrychlení \\\hline
        $ m $ & 0.03 kg & Hmotnost kolečka \\\hline
        $ R $ & 0.04 m & Poloměr kolečka \\\hline
        $ J_w $ & $\frac{mR^2}{2}$ kg . m 2 & Moment setrvačnosti kolečka \\\hline
        $ M $ & 0.6 kg & Hmotnost segwaye \\\hline
        $ W $ & 0.14 m & Šířka segwaye (délka ve směru osy rotace kol) \\\hline
        $ D $ & 0.04 m & Tloušťka segwaye (délka ve směru pohybu) \\\hline
        $ H $ & 0.144m & Výška  segwaye \\\hline
        $ L $ & $\frac{H}{2}$ m & Vzdálenost těžiště segwaye od osy kol \\\hline
        $ J_\psi $ & $\frac{ML^2}{3}$ kg . m 2 & Moment setrvačnosti náklonu segwaye\\\hline
        $ J_\varPhi $ & $\frac{M(W^2 + D^2)}{12}$ kg . m 2 & Moment setrvačnosti rotace segwaye \\\hline
        $ J_m $ & $10^{-5}$ kg . m 2 & Moment setrvačnosti motoru \\\hline
        $ R_m $ & 6.69 $\Omega$ & Odpor motoru \\\hline
        $ K_b $ & 0.468 V.s/rad & EMF kontanta motoru \\\hline
        $ K_t $ & 0.317 Nm/A & Konstanta kroutivého momentu motoru \\\hline
...
\begin{equation}
    \left[(2m + M) R^2 + 2 J_w + 2n^2 J_m\right] \ddot{\theta} + (MLR \text{cos} \psi - 2n^2 J_m) \ddot{\psi} - MLR\dot{\psi}^2 \text{sin} \psi = F_\theta
    \label{eq:pohyb1}
\end{equation}
\begin{equation}
    (MLR \text{cos} \psi - 2n^2 J_m) \ddot{\theta} + (ML^2 + J_\psi + 2n^2 J_m) \ddot{\psi} - MgL\text{sin}\psi - ML^2\dot{\varPhi}^2 \text{sin} \psi \text{cos} \psi = F_\psi
    \label{eq:pohyb2}
\end{equation}
\begin{equation}
    \left[ \frac{1}{2} mW^2  + J_\varPhi + \frac{W^2}{2R^2}\left( J_w + n^2 J_m \right) + ML^2 \text{sin}^2 \psi \right] \ddot{\varPhi} + 2ML^2 \dot{\psi} \dot{\varPhi} \text{sin}\psi \text{cos} \psi = F_\varPhi
    \label{eq:pohyb3}
\end{equation}

Zobecnělé síly na pravé straně rovnic lze vyjádřit jako
\begin{equation}
...
Bez újmy na obecnosti lez postavit dokonce $\varPhi = 0$ (odtud i $\dot{\varPhi} = \ddot{\varPhi} = 0$),
...
Protože platí vztah 
\begin{equation}
    d(t) = \int_{-\infty}^t v(t) \text{d}t = \int_ {-\infty}^t \dot{\theta}(t) \cdot R \text{d}t = R \cdot \theta(t),
    \label{eq:vzdalenost}
\end{equation}
...
\end{equation}
\begin{equation}
    (MLR \text{cos} \psi - 2n^2 J_m) \ddot{\theta} + (ML^2 + J_\psi + 2n^2 J_m) \ddot{\psi} - MgL\text{sin}\psi = F_\psi
...
V okolí pracovního bodu lze provést aproximaci $\text{cos}(\psi) \to 1$, $\text{sin}(\psi) \to \psi$. Rovněž výraz $\dot{\psi}^2$ se bude limitně blížit nule,
...
K tomuto rovnovážnému bodu přísluší stavový vektor
\begin{equation}
...
\begin{equation}
    ...
    \begin{equation}
        ...
        \end{bmatrix}.
    \end{equation}
Tím je modelování systému hotovo.


\begin{thebibliography}{9}

    \bibitem{model_based_design}
    Yorihisa Yamamoto , Supplementary lectures to book \emph{NXTway-GS Model-Based Design} 
    \end{thebibliography}
